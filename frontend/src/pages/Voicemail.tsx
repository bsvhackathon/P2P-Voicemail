...
const Voicemail: React.FC = () => {
  ...
  const fetchArchivedVoicemails = async () => {
    ...
    try {
      // Get all transactions in the voicemail basket, including spent ones
      const allVoicemails = await walletClient.listOutputs({
        basket: 'p2p voicemail',
        include: 'entire transactions'
      })
      
      // Filter out unspent transactions (these are in the regular inbox)
      // Use type assertion to access the spentAt property
      const spentVoicemails = allVoicemails.outputs.filter(output => 
        (output as any).spentAt !== undefined
      )
      
      console.log('Found spent voicemails:', spentVoicemails.length)
      
      // Decrypt each archived voicemail
      const decryptedArchivedVoicemails = await Promise.all(
        spentVoicemails.map(async (voicemail: any, i: number) => {
          try {
            const txid = voicemail.outpoint.split('.')[0]
            const tx = Transaction.fromBEEF(allVoicemails.BEEF as number[], txid)
            const lockingScript = tx!.outputs[0].lockingScript
            ...
            // Get redemption time if available
            const redemptionTime = voicemail.spentAt ? new Date(voicemail.spentAt).getTime() : null
      ...
      validArchivedVoicemails.sort((a, b) => {
        const timeA = a.redemptionTime || 0
        const timeB = b.redemptionTime || 0
        return timeB - timeA
      })
  ...
  const processRedemption = async () => {
    ...
    try {
      ...
      setNotification({
        open: true,
        message: `Successfully redeemed ${selectedVoicemail.satoshis} satoshis!`,
        type: 'success',
        title: 'Success'
      });
    } catch (error) {
  ...
  const processForgetContact = async (contact: { identityKey: string; name: string }) => {
    try {
      const response = await fetch('http://localhost:3321/forget-contact', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to forget contact');
      }
          ...
          <Card>
            <CardContent>
              ...
              {isLoadingArchived ? (
                ...
                <List>
                  {archivedVoicemails.map((voicemail) => (
                    <React.Fragment key={voicemail.id}>
                      <ListItem alignItems="flex-start">
                        <ListItemText
                          primary={
